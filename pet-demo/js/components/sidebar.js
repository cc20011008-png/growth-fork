...
export function renderSidebar(state, { active = "plan" } = {}) {
  ...
  host.innerHTML = `
    ...
    <nav class="side-nav">
      <a class="${active === "plan" ? "on" : ""}" href="./index.html">${ICONS.today}<span>今日</span></a>
      <a class="${active === "learn" ? "on" : ""}" href="./calendar.html">${ICONS.learn}<span>学习</span></a>
      <a class="${active === "goals" ? "on" : ""}" href="./goals.html">${ICONS.project}<span>项目</span></a>
      <a class="${active === "stats" ? "on" : ""}" href="./report.html">${ICONS.review}<span>复盘</span></a>
      <a href="../my-growth.html">${ICONS.inbox}<span>资源</span></a>
      <button type="button" id="open-bond">${ICONS.star}<span>成就</span></button>
    </nav>
    <div class="side-pet">
